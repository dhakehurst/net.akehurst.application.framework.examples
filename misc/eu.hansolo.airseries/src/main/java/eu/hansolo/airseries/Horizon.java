...
public class Horizon extends Region {
    ...
    Rotate getHorizonRotate() {
   		return this.horizonRotate;
    }
    ...
    public Horizon() {
        ...
        roll = new DoublePropertyBase(0) {
            @Override public void set(final double ROLL) {
            	double roll = ROLL % 360;
            	roll = (roll > 180) ? -(360 - roll) : roll;
            	super.set(roll);
            }
        ...
        pitch = new DoublePropertyBase(0) {
            @Override public void set(final double PITCH) {
                double pitch = PITCH % 360;
                if (pitch > 90 && pitch < 270) {
                    //pitch = 90 - (pitch - 90);
                    //if (!upsidedown.get()) {
                	//setRoll(roll.get() - 180);
                	//}
                	pitch = 180 - pitch;
                    setUpsideDown(true);
                } else if (pitch < -90) {
    ...
    private void initGraphics() {
        ...
        horizonCanvas.getTransforms().addAll(this.getHorizonRotate());
        ...
        arrow.getTransforms().add(this.getHorizonRotate());
    ...
    private void registerListeners() {
        ...
        upsideDownProperty().addListener(observable -> handleControlPropertyChanged("UPSIDEDOWN"));
    }
    ...
    private void handleControlPropertyChanged(final String PROPERTY) {
        if ("RESIZE".equals(PROPERTY)) {
            ...
            if (isAnimated()) {
                ...
                KeyValue kvRollBegin = new KeyValue(getHorizonRotate().angleProperty(), getHorizonRotate().getAngle(), Interpolator.EASE_IN);                
                KeyValue kvRollEnd   = new KeyValue(getHorizonRotate().angleProperty(), getRoll(), Interpolator.EASE_OUT);
        ...
        } else if ("UPSIDEDOWN".equals(PROPERTY)) {
        	double rollValue = this.isUpsideDown() ? this.getRoll() - 180 : this.getRoll();
            if (isAnimated()) {
                pitchPixel = Math.PI * size / 360;
                KeyValue kvRollBegin = new KeyValue(getHorizonRotate().angleProperty(), getHorizonRotate().getAngle(), Interpolator.EASE_IN);                
                KeyValue kvRollEnd   = new KeyValue(getHorizonRotate().angleProperty(), rollValue, Interpolator.EASE_OUT);
                KeyFrame kfBegin     = new KeyFrame(Duration.ZERO, kvRollBegin);
                KeyFrame kfEnd       = new KeyFrame(Duration.millis(ANIMATION_DURATION), kvRollEnd);
                timelineRoll.getKeyFrames().setAll(kfBegin, kfEnd);                
                timelineRoll.play();
            } else {
                horizonRotate.setAngle(rollValue);                
            }  
        }
    }
