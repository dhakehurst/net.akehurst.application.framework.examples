...
public class AirPressureSensor implements Runnable, IHardwareRegister {

	public AirPressureSensor(double sensingRatePerSecond) {
	...
	void simulateSampleAirPressure() {
		double d = this.environment.getAirPressure().asPrimitive();
	...
	public void run() {
		try {
			while(true) {
				this.simulateSampleAirPressure();
