...
public class EngineRequestProxy implements ISubscriber, Runnable {

	Logger log;
	
	public EngineRequestProxy() {
	...
	IHardwareRegister engineThrottleActuator;
	public IHardwareRegister getEngineThrottleActuator() {
		return engineThrottleActuator;
	}
	public void setEngineThrottleActuator(IHardwareRegister value) {
		this.engineThrottleActuator = value;
	}
	...
	public void update(byte[] bytes) {
		...
		if (data instanceof EngineThrottlePosition) {
			EngineThrottlePosition value = (EngineThrottlePosition)data;
			ByteBuffer bb = ByteBuffer.allocate(Double.BYTES);
			bb.putDouble(value.asPrimitive());
			this.getEngineThrottleActuator().writeValue(bb.array());
		} else {
	...
	public void start() {
		this.thread = new Thread(this, "EngineRequestSubscriber");
		this.thread.start();
	}
